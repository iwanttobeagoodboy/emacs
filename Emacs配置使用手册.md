...
```elisp
M-x my-verify-optimizations  ; 验证所有优化配置
...
### 全局前缀键
所有用户自定义快捷键均以 `C-c` (Ctrl+C) 作为前缀键。

> **提示**: 按下 `C-c` 后等待 0.3 秒，which-key 插件会自动显示所有可用快捷键。
...
| `C-c f r` | 打开最近文件 | `recentf-open-files` |
| `C-c b b` | 切换 Buffer | `switch-to-buffer` |
| `C-c b k` | 关闭当前 Buffer | `kill-buffer` |
| `C-c b r` | 重新加载当前文件 | `revert-buffer` |
| `C-c w v` | 垂直分割窗口 | `split-window-right` |
| `C-c w s` | 水平分割窗口 | `split-window-below` |
| `C-c w d` | 删除当前窗口 | `delete-window` |
| `C-c w o` | 最大化当前窗口 | `delete-other-windows` |

### 📦 2. 项目管理

| 快捷键 | 功能 | 命令 |
|--------|------|------|
| `C-c p f` | 项目中搜索文件 | `project-find-file` |
| `C-c p p` | 切换项目 | `project-switch-project` |
| `C-c p s` | 项目内容搜索 | `project-search` |
...
| 快捷键 | 功能 | 命令 |
|--------|------|------|
| `C-c c d` | 跳转到定义 | `xref-find-definitions` |
| `C-c c R` | 查找所有引用 | `xref-find-references` |
| `C-c c r` | 重命名变量 | `eglot-rename` |
| `C-c c a` | 代码操作菜单 | `eglot-code-actions` |
| `C-c c f` | 格式化文档 | `eglot-format` |
| `C-c c e` | 显示项目诊断 | `flymake-show-project-diagnostics` |
| `C-c c n` | 下一个错误 | `flymake-goto-next-error` |
| `C-c c p` | 上一个错误 | `flymake-goto-prev-error` |

### 🌿 4. Git 版本控制

| 快捷键 | 功能 | 命令 |
|--------|------|------|
| `C-c g g` | Magit 状态总览 | `magit-status` |
| `C-c g c` | 提交更改 | `magit-commit` |
| `C-c g p` | 推送到远程 | `magit-push` |
| `C-c g l` | 查看文件历史 | `magit-log-current` |
...
- **手动切换**：使用 `C-c v` 切换Vim模式开关
- **状态显示**：模式行显示当前状态（NORMAL/INSERT/VISUAL）

#### 5.3 模式切换
| 快捷键 | 功能 | 说明 |
|--------|------|------|
| `C-c v` | 切换Vim模式 | 全局启用/禁用Vim模式 |
| `i` | 进入插入模式 | 在光标前插入 |
| `a` | 进入插入模式 | 在光标后插入 |
...
| 快捷键 | 功能 | 命令 |
|--------|------|------|
| `C-c a c` | 启动 GPT 聊天 | `gptel` |
| `C-c a r` | AI 代码审查 | `my-ai-code-review` |
| `C-c a e` | 解释选中代码 | `my-ai-explain-code` |
| `C-c a d` | 生成文档字符串 | `my-ai-generate-docstring` |
| `C-c a m` | GPT 配置面板 | `gptel-menu` |

### 📝 7. Org 模式

| 快捷键 | 功能 | 命令 |
|--------|------|------|
| `C-c o c` | 捕获任务/笔记 | `org-capture` |
| `C-c o a` | 打开日程表 | `org-agenda` |
| `C-c o r` | Org-roam 查找 | `org-roam-node-find` |
| `C-c o h` | 发布到 Hugo | `my-org-publish-hugo` |
| `C-c o v` | 导出为 Reveal.js | `my-org-export-reveal` |

### 🖵 8. 终端

| 快捷键 | 功能 | 命令 |
|--------|------|------|
| `C-c t t` | 启动 Vterm 终端 | `vterm` |
| `C-c t s` | 启动 Eshell | `eshell` |
| `C-c t p` | 项目根目录启动 Eshell | `project-eshell` |

### 🆘 9. 帮助系统

| 快捷键 | 功能 | 命令 |
|--------|------|------|
| `C-c h k` | 显示键位帮助 | `my-describe-keybindings` |
| `C-c h b` | 显示缓冲区帮助 | `describe-buffer` |
| `C-c h c` | 显示补全后端 | `my-describe-completion-backends` |
| `C-c v o` | 验证优化配置 | `my-verify-optimizations` |
| `C-h V` | 验证Vim模式配置 | `my-verify-vim-config` |
...
### 补全系统 (Completion)
- **前端**: corfu（弹出式补全界面）
- **后端**: cape（多源补全：字典、文件、符号、历史等）
- **AI 补全**: copilot.el（GitHub Copilot）
- **使用技巧**:
  - 输入时自动触发补全
  - `TAB` 选择补全项
  - `M-n`/`M-p` 在补全项间导航
...
- 检查配置加载：`M-x my-verify-optimizations`
- 查看模块是否启用

#### 3. 键位冲突
- 使用 `C-c h k` 查看当前键位绑定
- 检查 `init-local.el` 中的自定义绑定
